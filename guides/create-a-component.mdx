...
The `name` and `description` will be visible on the Components page in the Architect UI once the
component is registered. The `name` property is also how components specify other components as dependencies.
...
from, relative to the `architect.yml` file. In this case, the path is `.` since the `architect.yml` file is in the
...
This section sets the `NODE_ENV` environment variable is set to `production`. This variable is used in the Dockerfile
...
architect dev
```
Once the containers are running, you will be able to see your frontend at https://app.localhost.architect.sh.
Notice that the subdomain `app` is coming from our defined ingress subdomain.

### Registering a component

You must register a component before you can deploy it to the Architect cloud or an external cluster. You'll start by
logging in.
