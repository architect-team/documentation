...
1. **Component deployment**: Whenever a developer deploys a Component to an Environment, Architect
   ...
   Environment configuration is stored in the [state backend](/docs/cli/managing-state) the CLI has
...
3. **Datacenter enrichment**: Armed with a graph of resources that should exist in the environment,
   ...
   resource. Each resource gets matched against a [hook](/docs/datacenters/hooks) in the datacenter
   that tells what modules to execute, how to configure them, and how to extract the output values
   needed for any resources that depend on it.
   - _The graph produced as part of resource extraction represents a contract between the Components
     ...
     [supported resources](/docs/datacenters/hooks), and the edges help determine which order to
     deploy the resources in. This helps Datacenters control the infrastructure without getting into
     the weeds of which resources depend on which others._
