...
Architect uses Traefik as the reverse proxy to handle incoming traffic to all services. When a component is deployed, the graph automatically generates the gateway node.
Traefik dynamically discovers and routes traffic based on the labels applied to the services. These labels are added during the deployments to configure both the Traefik and the Cert Manager. 
