...
const AdmonitionConfigs = {
  ...
  tip: {
    ...
    contentClassName: 'text-green-700',
...
export default function Admonition(props) {
  ...
  const icon = iconProp ?? (
    <IconComponent className={clsx('h-5 w-5', typeConfig.iconClassName)} aria-hidden="true" />
  );

  return (
    <p className={clsx('not-prose rounded-md p-4', typeConfig.wrapperClassName)}>
      <div className="flex">
        <div className="flex-shrink-0">{icon}</div>
        <div className="ml-3">
          {title && (
            <h3 className={clsx('text-sm font-medium', typeConfig.titleClassName)}>{title}</h3>
          )}

          <div
            className={clsx('text-sm', typeConfig.contentClassName, {
              'mt-2': Boolean(title),
            })}
          >
            <p>{children}</p>
